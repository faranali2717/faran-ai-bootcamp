...
y_train_encoded = one_hot(y_train_raw)
y_test = one_hot(y_test_raw)

# Validation Split
# 50,000 for training, 10,000 for validation monitoring
val_size = 10000
...
y_train = y_train_encoded[:-val_size]
...
for epoch in range(epochs):
  ...
  val_acc = compute_accuracy(a2_val, y_val_raw) * 100
  print(
      f'epoch {epoch+1:02d}/{epochs:02d} :: loss: {average_loss:.4f} :: val'
...
print(f'\nfinal testing accuracy on unseen test set: {final_test_acc:.2f}%')

# ploting  results
plt.figure(figsize=(8, 5))
plt.plot(
    range(1, epochs + 1), loss_history, marker='o', color='b', linewidth=2
)
plt.title('training loss curve (optimized numpy mlp)')
plt.xlabel('epoch')
